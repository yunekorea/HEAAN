...
int main(int argc, char **argv) {
  ...
  */
  ...
  std::string temp;
  char* input_0_path = WorkingDir.data();
  char* input_0_filename = NULL;
  char* input_1_path = WorkingDir.data();
  char* input_1_filename = NULL;
  char* target_path = WorkingDir.data();
  char* target_filename = NULL;
  //complex<double>* dvecadd[iter];
  for(int i = 0; i < iter; i++) { 
    cout << "Progress : " << i << "/" << iter-1 << "\r";
    
    temp = "/1GB_ciphertexts/" + SFileName0 + std::to_string(i) + ".cip";
    input_0_filename = temp.data();
    temp = "/1GB_ciphertexts/" + SFileName1 + std::to_string(i) + ".cip";
    input_1_filename = temp.data();
    temp = "/1GB_ciphertexts/" + SFileNameAdd + std::to_string(i) + ".cip";
    target_filename = temp.data();
